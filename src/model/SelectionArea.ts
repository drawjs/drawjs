...
import { selectCell, enableCellDrag } from "mixin/index"
...
export default class SelectionArea extends Geometry {
	...
	public _mousedownListener = event => {
		...
		if (
			this._isPointOnCell( event ) &&
			this._isPointOnUnselectedCell( event )
		) {
			this._unselectCells()
		...
		if ( this._isPointOnCell( event ) && this._isPointOnSelectedCell( event ) ) {
